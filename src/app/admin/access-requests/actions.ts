...
export async function approveRequest(id: string, email: string, name: string): Promise<{ success: boolean; error?: string; warning?: string }> {
  try {
    await db
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        warning = 'User approved but notification email failed to send.';
      }
    }
...
export async function denyRequest(id: string, email: string, name: string): Promise<{ success: boolean; error?: string; warning?: string }> {
  try {
    await db
      .update(accessRequests)
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        warning = 'Request denied but notification email failed to send.';
      }
    }
